    ...
    items.forEach(dept => {
        ...
        item.addEventListener('mousedown', (e) => {
            ...
            loadStations(currentDepartmentOid).then(() => {
                ...
                // dateRange stays disabled until station is selected
            });
        });
...
function checkAndSearchHistoryPrintByStation() {
    ...
    SearchHistoryPrintByStation(station, fromDate, toDate);
}

async function SearchHistoryPrintByStation(station, fromDate, toDate) {
    if (!station) {
        clearTable();
        return;
    }
