...
async function SearchHistoryScanByStation(station, fromDate, toDate) {
    ...
    try {
        ...
        if (!data || !data.result || data.result.length === 0) {
            setTableData([], data ? data.columns : [], null, `Máy ${station} không có lịch sử quét tem${dateMsg}`);
            return;
        }

        setTableData(data.result, data.columns, null, null, `Tìm thấy ${data.result.length} bản ghi lịch sử quét tem`);
    } catch (error) {
        console.error('Error searching scan barcode history:', error);
        clearTable();
    }
}
